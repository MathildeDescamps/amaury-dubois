...
if (!class_exists('OneaElatedClassTopNavigationWalker')) {
	class OneaElatedClassTopNavigationWalker extends Walker_Nav_Menu {
		...
		function start_lvl( &$output, $depth = 0, $args = array() ) {
			...
			if($depth == 0){
				$out_div = '<div class="second '.$background_image_class.'"><div class="inner" '.($menu_image_background != '' ? onea_elated_get_inline_style($menu_image_background) : '').'>';
			}else{
				$out_div = '';
			}
		...
		function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			if($depth==0){
				if($item->type_menu == "wide"){
					$menu_type = " wide";
				} else if( $item->type_menu == "wide-1-5" ) {
					$menu_type = " wide wide-1-5";
				} else {
					$menu_type = " narrow";
				}
			}
			...
			if($item->hide == ""){
				...
				if($args->has_children && $depth == 0) {
					$item_output .= '<i class="eltdf-menu-arrow eltdf-icon-ion-icon ion-chevron-down"></i>';
				}
