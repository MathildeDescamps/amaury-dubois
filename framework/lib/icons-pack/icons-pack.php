...
foreach ( glob( ELATED_FRAMEWORK_ICONS_ROOT_DIR . '/*/load.php' ) as $module_load ) {
	if ( onea_elated_is_customizer_item_enabled( $module_load, 'onea_performance_disable_icon_pack_' ) ) {
		include_once $module_load;
	}
}

if ( ! function_exists( 'onea_elated_get_icon_pack_collections' ) ) {
	function onea_elated_get_icon_pack_collections() {
		$options = apply_filters( 'onea_elated_filter_add_icon_pack_into_collection', $options = array() );
		
		return $options;
	}
	
	add_action( 'after_setup_theme', 'onea_elated_get_icon_pack_collections' );
}

if ( ! function_exists( 'onea_elated_get_icon_pack_options' ) ) {
	function onea_elated_get_icon_pack_options() {
		$options = apply_filters( 'onea_elated_filter_add_icon_pack_into_options', $options = array() );
		
		return $options;
	}
	
	add_action( 'after_setup_theme', 'onea_elated_get_icon_pack_options' );
}

if ( ! function_exists( 'onea_elated_get_icon_pack_social_options' ) ) {
	function onea_elated_get_icon_pack_social_options() {
		$options = apply_filters( 'onea_elated_filter_add_icon_pack_into_social_options', $options = array() );
		
		return $options;
	}
	
	add_action( 'after_setup_theme', 'onea_elated_get_icon_pack_social_options' );
}

/*
 * Class: OneaElatedClassIconCollections
 */
class OneaElatedClassIconCollections {
	...
	private function __construct() {
		...
		$this->iconPackCollections   = onea_elated_get_icon_pack_collections();
		$this->iconPackOptions       = onea_elated_get_icon_pack_options();
		$this->iconPackSocialOptions = onea_elated_get_icon_pack_social_options();
	...
	public function getIconsMetaBoxOrOption( $attributes ) {
		...
		foreach ( $icon_collections as $collection_key => $collection_object ) {
			...
			$icon_pack_container = onea_elated_add_admin_container(
				array(
					'parent'     => $parent,
	...
	public function getVCParamsArray( $iconPackDependency = array(), $iconCollectionPrefix = "", $emptyIconPack = false ) {
		...
		$iconPackParams = array(
			'type'        => 'dropdown',
			'heading'     => esc_html__( 'Icon Pack', 'onea' ),
	...
	public function getIconWidgetParamsArray() {
		$iconPackParams[] = array(
			'type'    => 'dropdown',
			'name'    => 'icon_pack',
			'title'   => esc_html__( 'Icon Pack', 'onea' ),
			'options' => $this->getIconPackOptions()
		);
		...
		if ( is_array( $this->iconCollections ) && count( $this->iconCollections ) ) {
			foreach ( $this->iconCollections as $key => $collection ) {
				$iconSetParams[] = array(
					'type'    => 'dropdown',
					'title'   => $collection->title . esc_html__( ' Icon', 'onea' ),
	...
	public function getSocialIconWidgetMultipleParamsArray( $count ) {
		...
		for ( $n = 1; $n <= $count; $n ++ ) {
			if ( is_array( $iconCollectionsVC ) && count( $iconCollectionsVC ) ) {
				foreach ( $iconCollectionsVC as $key => $collection ) {
					$iconOps[] = array(
						...
						'title'   => sprintf( esc_html__( 'Icon %s %s Icon', 'onea' ), $n, $collection->title ),
			...
			$iconOps[] = array(
				'type'  => 'textfield',
				'name'  => 'link_' . $n,
				'title' => sprintf( esc_html__( 'Link %s', 'onea' ), $n )
			);
			
			$iconOps[] = array(
				'type'    => 'dropdown',
				'name'    => 'target_' . $n,
				'title'   => sprintf( esc_html__( 'Link Target %s', 'onea' ), $n ),
	...
	public function getIconSizesArray() {
		return array(
			esc_html__( 'Tiny', 'onea' )       => 'fa-lg',
			esc_html__( 'Small', 'onea' )      => 'fa-2x',
			esc_html__( 'Medium', 'onea' )     => 'fa-3x',
			esc_html__( 'Large', 'onea' )      => 'fa-4x',
			esc_html__( 'Very Large', 'onea' ) => 'fa-5x'
		);
	}
	...
	public function getIconCollectionIcons( iOneaElatedInterfaceIconCollection $collection ) {
	...
	public function getIconCollectionsEmpty( $no_empty_key = "" ) {
		$array                  = array();
		$array[ $no_empty_key ] = esc_html__( 'No Icon', 'onea' );
	...
	public function getIconCollectionsVCEmpty() {
		$vc_array                                           = array();
		$vc_array[ esc_html__( 'No Icon', 'onea' ) ] = '';
