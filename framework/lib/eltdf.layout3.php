...
/*
   Class: OneaElatedClassMultipleImages
   A class that initializes Elated Multiple Images
*/

class OneaElatedClassMultipleImages implements iOneaElatedInterfaceRender {
	private $name;
	private $label;
	private $description;
	...
	function __construct( $name, $label = "", $description = "" ) {
		...
		$onea_elated_global_Framework->eltdMetaBoxes->addOption( $this->name, "" );
	}
					...
					<div class="row">
						<div class="col-lg-12">
							...
							<div class="eltdf-gallery-uploader">
								<a class="eltdf-gallery-upload-btn btn btn-sm btn-primary" href="javascript:void(0)"><?php esc_html_e( 'Upload', 'onea' ); ?></a>
								<a class="eltdf-gallery-clear-btn btn btn-sm btn-default pull-right" href="javascript:void(0)"><?php esc_html_e( 'Remove All', 'onea' ); ?></a>
							</div>
						</div>
					</div>
				</div>
...
class OneaElatedClassTwitterFramework implements iOneaElatedInterfaceRender {
	public function render( $factory ) {
		$twitterApi = OneaTwitterApi::getInstance();
		$message    = '';
		...
		if ( ! empty( $_GET['oauth_token'] ) && ! empty( $_GET['oauth_verifier'] ) ) {
			...
			if ( $responseObj->status ) {
				$message = esc_html__( 'You have successfully connected with your Twitter account. If you have any issues fetching data from Twitter try reconnecting.', 'onea' );
			} else {
				$message = $responseObj->message;
			}
		}
		
		$buttonText = $twitterApi->hasUserConnected() ? esc_html__( 'Re-connect with Twitter', 'onea' ) : esc_html__( 'Connect with Twitter', 'onea' );
		?>
		<?php if ( $message !== '' ) { ?>
		...
		<div class="eltdf-page-form-section" id="eltdf_enable_social_share">
			<div class="eltdf-field-desc">
				<h4><?php esc_html_e( 'Connect with Twitter', 'onea' ); ?></h4>
				<p><?php esc_html_e( 'Connecting with Twitter will enable you to show your latest tweets on your site', 'onea' ); ?></p>
			</div>
			<div class="eltdf-section-content">
				<div class="container-fluid">
					<div class="row">
...
class OneaElatedClassInstagramFramework implements iOneaElatedInterfaceRender {
	public function render( $factory ) {
		...
		if ( ! empty( $_GET['code'] ) && ! empty( $_GET['instagram'] ) ) {
			...
			$message = esc_html__( 'You have successfully connected with your Instagram Personal account.', 'onea' );
		}
		...
		if ( ! empty( $_GET['access_token'] ) && ! empty( $_GET['facebook'] ) ) {
			...
			$message = esc_html__( 'You have successfully connected with your Instagram Business account.', 'onea' );
		}
		...
		if ( ! empty( $_GET['disconnect'] ) ) {
			...
			$message = esc_html__( 'You have have been disconnected from all Instagram accounts.', 'onea' );
		...
		<div class="eltdf-page-form-section" id="eltdf_enable_social_share">
			<div class="eltdf-field-desc">
				<h4><?php esc_html_e( 'Connect with Instagram', 'onea' ); ?></h4>
				<p><?php esc_html_e( 'Connecting with Instagram will enable you to show your latest photos on your site', 'onea' ); ?></p>
			</div>
			<div class="eltdf-section-content">
				<div class="container-fluid">
					<?php
					...
					if ( $instagram_user_id ) { ?>
						<div class="row">
							<div class="col-lg-12">
								<p><?php echo esc_html__( 'You are currently connected to Instagram ID: ', 'onea' );
					...
					<div class="row">
						<?php if ( ! empty( $_GET['disconnect'] ) ) { ?>
							<div class="col-lg-4">
								<a class="btn btn-primary" href="<?php echo esc_url( $instagram_api->reloadURL() ); ?>"><?php echo esc_html__( 'Reload Page', 'onea' ); ?></a>
							</div>
						<?php } else if ( empty( $connection_type ) ) { ?>
							<div class="col-lg-4">
								<a class="btn btn-primary" href="<?php echo esc_url( $instagram_api->instagramRequestCode() ); ?>"><?php echo esc_html__( 'Connect with Instagram Personal account', 'onea' ); ?></a>
							</div>
							<div class="col-lg-4">
								<a class="btn btn-primary" href="<?php echo esc_url( $instagram_api->facebookRequestCode() ); ?>"><?php echo esc_html__( 'Connect with Instagram Business account', 'onea' ); ?></a>
							</div>
						<?php } else { ?>
							<div class="col-lg-4">
								<a class="btn btn-primary" href="<?php echo esc_url( $instagram_api->disconnectURL() ); ?>"><?php echo esc_html__( 'Disconnect Instagram account', 'onea' ) ?></a>
							</div>
						<?php } ?>
					</div>
				</div>
...
class OneaElatedClassRepeater implements iOneaElatedInterfaceRender {
	private $label;
	private $description;
	private $name;
	private $fields;
	...
	function __construct( $fields, $name, $label = '', $description = '', $button_text = '', $table_layout = false ) {
		...
		$this->name         = $name;
		$this->num_of_rows  = 1;
		$this->button_text  = ! empty( $button_text ) ? $button_text : esc_html__( 'Add New Item', 'onea' );
			...
			<?php if ( $this->table_layout ) { ?>
				<div class="eltdf-repeater-table-heading">
					<div class="eltdf-repeater-fields-holder">
						<div class="eltdf-repeater-table-cell eltdf-repeater-sort"><?php esc_html_e( 'Order', 'onea' ) ?></div>
						...
						<div class="eltdf-repeater-table-cell eltdf-repeater-remove"><?php esc_html_e( 'Remove', 'onea' ) ?></div>
					</div>
				</div>
			<?php } ?>
																			...
																			<div class="eltdf-repeater-inner-fields-row <?php echo esc_attr( $col_width_inner_class ); ?>">
																				<div class="eltdf-repeater-inner-fields-row-inner">
																					...
																					if ( ! empty( $field_inner['dependency'] ) ) {
																						$dependencyValues = onea_elated_return_repeater_dependency_options_array(array(
																							'field' 	   => $field,
...
class OneaElatedClassFieldAddress extends OneaElatedClassFieldType {
					...
					<div class="row">
						<div class="col-lg-<?php echo esc_attr( $col_width ); ?>">
							...
							if ( empty( $google_maps_api_key ) ) { ?>
								<p class="description"><?php esc_html_e( 'In order for the map functionality to be enabled please input the Google Map API key in the General section of the Onea Options', 'onea' ); ?></p>
							<?php } ?>
							<a class="eltdf-reset-marker eltdf-hide-field" href="#"><?php esc_html_e( 'Reset Marker', 'onea' ); ?></a>
...
class OneaElatedClassFieldIcon extends OneaElatedClassFieldType {
	public function render( $name, $label = "", $description = "", $options = array(), $args = array(), $repeat = array() ) {
		$class = '';
		...
		$icon_packs        = onea_elated_icon_collections()->getIconCollectionsEmpty();
		$icons_collections = onea_elated_icon_collections()->getIconCollectionsKeys();
		?>
			...
			<div class="eltdf-section-content">
				<div class="container-fluid">
					...
					<?php foreach ( $icons_collections as $icons_collection ) { ?>
						<?php
						$icons_param = onea_elated_icon_collections()->getIconCollectionParamNameByKey( $icons_collection );
						...
						<div class="row eltdf-icon-collection-holder <?php echo esc_attr( $field_class ); ?>" data-icon-collection="<?php echo esc_attr( $icons_collection ); ?>">
							<div class="col-lg-<?php echo esc_attr( $col_width ); ?>">
								<select name="<?php echo esc_attr( $name . '[' . $icons_param . ']' ); ?>" class="<?php echo esc_attr( $select2 ) ?> form-control eltdf-form-element">
									<?php
									$icons       = onea_elated_icon_collections()->getIconCollection( $icons_collection );
