...
class ElatedSkin extends OneaElatedClassSkinAbstract {
	
    /**
     * Skin constructor. Hooks to onea_elated_admin_scripts_init and onea_elated_enqueue_admin_styles
     */
	public function __construct() {
		$this->skinName = 'elated';
		
		//hook to
		add_action( 'onea_elated_action_admin_scripts_init', array( $this, 'registerStyles' ) );
		add_action( 'onea_elated_action_admin_scripts_init', array( $this, 'registerScripts' ) );
		
		add_action( 'onea_elated_action_enqueue_admin_styles', array( $this, 'enqueueStyles' ) );
		add_action( 'onea_elated_action_enqueue_admin_scripts', array( $this, 'enqueueScripts' ) );
		
		add_action( 'onea_elated_action_enqueue_meta_box_styles', array( $this, 'enqueueStyles' ) );
		add_action( 'onea_elated_action_enqueue_meta_box_scripts', array( $this, 'enqueueScripts' ) );
		
		add_filter( 'onea_elated_filter_options_map_position', array( $this, 'setOptionsMapPosition' ), 10, 2 );
	...
	public function registerScripts() {
		...
		foreach ( $this->scripts as $scriptHandle => $script ) {
			onea_elated_register_skin_script( $scriptHandle, $script['path'], $script['dependency'] );
		}
	}
    ...
    public function registerStyles() {
        ...
        foreach ($this->styles as $styleHandle => $stylePath) {
	        onea_elated_register_skin_style($styleHandle, $stylePath);
        }
    }
    ...
    public function renderOptions() {
        ...
        $tab    = onea_elated_get_admin_tab();
        $active_page = $onea_elated_global_Framework->eltdOptions->getAdminPageFromSlug($tab);
     ...
     *
     * @see OneaElatedClassSkinAbstract::loadTemplatePart()
     */
	 ...
	 *
	 * @param OneaElatedClassAdminPage $page current page to load
	 * @see OneaElatedClassSkinAbstract::loadTemplatePart()
	 */
