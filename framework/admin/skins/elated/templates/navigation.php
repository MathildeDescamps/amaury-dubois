<div class="eltdf-tabs-navigation-wrapper">
    <ul class="nav nav-tabs">
        <?php
        foreach (onea_elated_options()->adminPages as $key => $page ) {
            $slug = "";
        ...
        <?php if (onea_elated_core_plugin_installed()) { ?>
			<li <?php if($is_backup_options_page) { echo "class='active'"; } ?>><a href="<?php echo esc_url(get_admin_url().'admin.php?page='.ELATED_OPTIONS_SLUG.'_backup_options'); ?>"><i class="fa fa-database eltdf-tooltip eltdf-inline-tooltip left" data-placement="top" data-toggle="tooltip" title="<?php esc_attr_e('Backup Options','onea'); ?>"></i><span><?php esc_html_e('Backup Options','onea'); ?></span></a></li>
        <?php } ?>
    </ul>
</div>