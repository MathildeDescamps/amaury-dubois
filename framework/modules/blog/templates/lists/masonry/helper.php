...
if ( ! function_exists( 'onea_elated_get_blog_holder_params' ) ) {
	...
	function onea_elated_get_blog_holder_params( $params ) {
		...
		$masonry_layout = onea_elated_get_meta_field_intersect( 'blog_masonry_layout' );
...
if ( ! function_exists( 'onea_elated_get_blog_list_classes' ) ) {
	/**
	 * Function that generates blog list holder classes for blog list templates
	 */
	function onea_elated_get_blog_list_classes( $classes ) {
		$list_classes   = array();
		$list_classes[] = 'eltdf-grid-list eltdf-grid-masonry-list';
		
		$number_of_columns = onea_elated_get_meta_field_intersect( 'blog_masonry_number_of_columns' );
		if ( ! empty( $number_of_columns ) ) {
			$list_classes[] = 'eltdf-' . $number_of_columns . '-columns';
		}
		
		$space_between_items = onea_elated_get_meta_field_intersect( 'blog_masonry_space_between_items' );
	...
	add_filter( 'onea_elated_filter_blog_list_classes', 'onea_elated_get_blog_list_classes' );
}
