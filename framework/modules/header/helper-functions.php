...
if ( ! function_exists( 'onea_elated_header_skin_class' ) ) {
	/**
	 * Function that adds header style class to body tag
	 */
	function onea_elated_header_skin_class( $classes ) {
		$header_style     = onea_elated_get_meta_field_intersect( 'header_style', onea_elated_get_page_id() );
		$header_style_404 = onea_elated_options()->getOptionValue( '404_header_style' );
	...
	add_filter( 'body_class', 'onea_elated_header_skin_class' );
}

if ( ! function_exists( 'onea_elated_sticky_header_behaviour_class' ) ) {
	/**
	 * Function that adds header behavior class to body tag
	 */
	function onea_elated_sticky_header_behaviour_class( $classes ) {
		$header_behavior = onea_elated_get_meta_field_intersect( 'header_behaviour', onea_elated_get_page_id() );
	...
	add_filter( 'body_class', 'onea_elated_sticky_header_behaviour_class' );
}

if ( ! function_exists( 'onea_elated_menu_dropdown_appearance' ) ) {
	/**
	 ...
	 */
	function onea_elated_menu_dropdown_appearance( $classes ) {
		$dropdown_menu_appearance = onea_elated_options()->getOptionValue( 'menu_dropdown_appearance' );
	...
	add_filter( 'body_class', 'onea_elated_menu_dropdown_appearance' );
}

if ( ! function_exists( 'onea_elated_header_class' ) ) {
	/**
	 ...
	 */
	function onea_elated_header_class( $classes ) {
		...
		$disable_menu_area_shadow = onea_elated_get_meta_field_intersect( 'menu_area_shadow', $id ) == 'no';
		if ( $disable_menu_area_shadow ) {
			$classes[] = 'eltdf-menu-area-shadow-disable';
		}
		
		$disable_menu_area_grid_shadow = onea_elated_get_meta_field_intersect( 'menu_area_in_grid_shadow', $id ) == 'no';
		if ( $disable_menu_area_grid_shadow ) {
			$classes[] = 'eltdf-menu-area-in-grid-shadow-disable';
		}
		
		$disable_menu_area_border = onea_elated_get_meta_field_intersect( 'menu_area_border', $id ) == 'no';
		if ( $disable_menu_area_border ) {
			$classes[] = 'eltdf-menu-area-border-disable';
		}
		
		$disable_menu_area_grid_border = onea_elated_get_meta_field_intersect( 'menu_area_in_grid_border', $id ) == 'no';
		if ( $disable_menu_area_grid_border ) {
			$classes[] = 'eltdf-menu-area-in-grid-border-disable';
		}
		
		if ( onea_elated_get_meta_field_intersect( 'menu_area_in_grid', $id ) == 'yes' &&
		     onea_elated_get_meta_field_intersect( 'menu_area_grid_background_color', $id ) !== '' &&
		     onea_elated_get_meta_field_intersect( 'menu_area_grid_background_transparency', $id ) !== '0'
		) {
			$classes[] = 'eltdf-header-menu-area-in-grid-padding';
		}
		
		$disable_logo_area_border = onea_elated_get_meta_field_intersect( 'logo_area_border', $id ) == 'no';
		if ( $disable_logo_area_border ) {
			$classes[] = 'eltdf-logo-area-border-disable';
		}
		
		$disable_logo_area_grid_border = onea_elated_get_meta_field_intersect( 'logo_area_in_grid_border', $id ) == 'no';
		if ( $disable_logo_area_grid_border ) {
			$classes[] = 'eltdf-logo-area-in-grid-border-disable';
		}
		
		if ( onea_elated_get_meta_field_intersect( 'logo_area_in_grid', $id ) == 'yes' &&
		     onea_elated_get_meta_field_intersect( 'logo_area_grid_background_color', $id ) !== '' &&
		     onea_elated_get_meta_field_intersect( 'logo_area_grid_background_transparency', $id ) !== '0'
		) {
			$classes[] = 'eltdf-header-logo-area-in-grid-padding';
		}
		
		$disable_shadow_vertical = onea_elated_get_meta_field_intersect( 'vertical_header_shadow', $id ) == 'no';
		if ( $disable_shadow_vertical ) {
			$classes[] = 'eltdf-header-vertical-shadow-disable';
		}
		
		$disable_border_vertical = onea_elated_get_meta_field_intersect( 'vertical_header_border', $id ) == 'no';
	...
	add_filter( 'body_class', 'onea_elated_header_class' );
}

if ( ! function_exists( 'onea_elated_header_area_style' ) ) {
	/**
	 * Function that return styles for header area
	 */
	function onea_elated_header_area_style( $style ) {
		...
		$menu_area_background_color_rgba = onea_elated_rgba_color( $menu_area_background_color, $menu_area_background_transparency );
		...
		if ( $menu_area_height !== '' ) {
			$menu_area_style['height'] = onea_elated_filter_px( $menu_area_height ) . 'px !important';
		}
		...
		if ( $container_side_padding !== '' ) {
			...
			$current_style .= onea_elated_dynamic_css( $menu_container_selector, $menu_container_styles );
		}
		...
		$menu_area_grid_background_color_rgba = onea_elated_rgba_color( $menu_area_grid_background_color, $menu_area_grid_background_transparency );
		
		if ( ! empty( $menu_area_grid_background_color_rgba ) ) {
			$menu_area_grid_style['background-color'] = $menu_area_grid_background_color_rgba;
		}
		
		$current_style .= onea_elated_dynamic_css( $menu_area_selector, $menu_area_style );
		$current_style .= onea_elated_dynamic_css( $menu_area_grid_selector, $menu_area_grid_style );
		...
		if ( $dropdown_top_position !== '' ) {
			$dropdown_styles['top'] = onea_elated_filter_suffix( $dropdown_top_position, '%' ) . '%';
		}
		
		$dropdown_selector = array( $class_prefix . ' .eltdf-page-header .eltdf-drop-down .second' );
		
		$current_style .= onea_elated_dynamic_css( $dropdown_selector, $dropdown_styles );
		...
		$logo_area_background_color_rgba = onea_elated_rgba_color( $logo_area_background_color, $logo_area_background_transparency );
		...
		if ( $logo_area_height !== '' ) {
			$logo_area_style['height'] = onea_elated_filter_px( $logo_area_height ) . 'px !important';
		}
		...
		$logo_area_grid_background_color_rgba = onea_elated_rgba_color( $logo_area_grid_background_color, $logo_area_grid_background_transparency );
		...
		if ( ! empty( $logo_area_style ) ) {
			$current_style .= onea_elated_dynamic_css( $logo_area_selector, $logo_area_style );
		}
		
		if ( ! empty( $logo_area_grid_style ) ) {
			$current_style .= onea_elated_dynamic_css( $logo_area_grid_selector, $logo_area_grid_style );
		}
		
		$current_style = apply_filters( 'onea_elated_filter_add_header_page_custom_style', $current_style, $class_prefix, $page_id ) . $style;
		
		return $current_style;
	}
	
	add_filter( 'onea_elated_filter_add_page_custom_style', 'onea_elated_header_area_style' );
}