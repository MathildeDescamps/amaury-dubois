...
class HeaderStandardExtended extends HeaderType {
	...
	public function __construct() {
		...
		if ( ! is_admin() ) {
			$this->logoAreaHeight     = onea_elated_set_default_logo_height_for_header_types();
			$this->menuAreaHeight     = onea_elated_set_default_menu_height_for_header_types();
	...
	public function loadTemplate( $parameters = array() ) {
		...
		$parameters['logo_area_in_grid'] = onea_elated_get_meta_field_intersect( 'logo_area_in_grid', $id ) == 'yes' ? true : false;
		$parameters['menu_area_in_grid'] = onea_elated_get_meta_field_intersect( 'menu_area_in_grid', $id ) == 'yes' ? true : false;
		
		$parameters = apply_filters( 'onea_elated_filter_header_standard_extended_parameters', $parameters );
	...
	public function calculateHeightOfTransparency() {
		...
		$logo_background_color             = onea_elated_get_meta_field_intersect( 'logo_area_background_color', $id );
		$logo_background_transparency      = onea_elated_get_meta_field_intersect( 'logo_area_background_transparency', $id );
		$logo_grid_background_color        = onea_elated_options()->getOptionValue( 'logo_area_grid_background_color' );
		$logo_grid_background_transparency = onea_elated_options()->getOptionValue( 'logo_area_grid_background_transparency' );
		...
		$menu_background_color             = onea_elated_get_meta_field_intersect( 'menu_area_background_color', $id );
		$menu_background_transparency      = onea_elated_get_meta_field_intersect( 'menu_area_background_transparency', $id );
		$menu_grid_background_color        = onea_elated_options()->getOptionValue( 'menu_area_grid_background_color' );
		$menu_grid_background_transparency = onea_elated_options()->getOptionValue( 'menu_area_grid_background_transparency' );
		...
		if ( $logoAreaTransparent || $menuAreaTransparent ) {
			if ( $logoAreaTransparent ) {
				...
				if ( ( $sliderExists && onea_elated_is_top_bar_enabled() )
				     || onea_elated_is_top_bar_enabled() && onea_elated_is_top_bar_transparent()
				) {
					$transparencyHeight += onea_elated_get_top_bar_height();
				}
			}
	...
	public function calculateHeightOfCompleteTransparency() {
		...
		$logo_background_color             = onea_elated_options()->getOptionValue( 'logo_area_background_color' );
		$logo_background_transparency      = onea_elated_options()->getOptionValue( 'logo_area_background_transparency' );
		$logo_grid_background_color        = onea_elated_options()->getOptionValue( 'logo_area_grid_background_color' );
		$logo_grid_background_transparency = onea_elated_options()->getOptionValue( 'logo_area_grid_background_transparency' );
		...
		$menu_background_color             = onea_elated_options()->getOptionValue( 'menu_area_background_color' );
		$menu_background_transparency      = onea_elated_options()->getOptionValue( 'menu_area_background_transparency' );
		$menu_grid_background_color        = onea_elated_options()->getOptionValue( 'menu_area_grid_background_color' );
		$menu_grid_background_transparency = onea_elated_options()->getOptionValue( 'menu_area_grid_background_transparency' );
		...
		if ( $logoAreaTransparent || $menuAreaTransparent ) {
			if ( $logoAreaTransparent ) {
				...
				if ( onea_elated_is_top_bar_enabled() && onea_elated_is_top_bar_completely_transparent() ) {
					$transparencyHeight += onea_elated_get_top_bar_height();
				}
			}
	...
	public function calculateHeaderHeight() {
		...
		if ( onea_elated_is_top_bar_enabled() ) {
			$headerHeight += onea_elated_get_top_bar_height();
		}
	...
	public function getPerPageJSVariables( $perPageVars ) {
		...
		$header_behavior = onea_elated_get_meta_field_intersect( 'header_behaviour' );
		if ( ! in_array( $header_behavior, array( 'sticky-header-on-scroll-up', 'sticky-header-on-scroll-down-up' ) ) ) {
			$perPageVars['eltdfHeaderTransparencyHeight'] = $this->headerHeight - ( onea_elated_get_top_bar_height() + $this->heightOfCompleteTransparency );
		} else {
