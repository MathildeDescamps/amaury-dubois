<?php
namespace OneaCore\CPT\Shortcodes\ProductInfo;
...
class ProductInfo implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if(function_exists('vc_map')) {
			vc_map(
				array(
					'name'                      => esc_html__( 'Product Info', 'onea' ),
					'base'                      => $this->getBase(),
					...
					'params'                    => array(
						...
						array(
							...
							'heading'     => esc_html__( 'Product Info Type', 'onea' ),
							'value'       => array(
								esc_html__( 'Title', 'onea' )          => 'title',
								esc_html__( 'Featured Image', 'onea' ) => 'featured_image',
								esc_html__( 'Category', 'onea' )       => 'category',
								esc_html__( 'Excerpt', 'onea' )        => 'excerpt',
								esc_html__( 'Price', 'onea' )          => 'price',
								esc_html__( 'Rating', 'onea' )         => 'rating',
								esc_html__( 'Add To Cart', 'onea' )    => 'add_to_cart',
								esc_html__( 'Tag', 'onea' )            => 'tag',
								esc_html__( 'Author', 'onea' )         => 'author',
						...
						array(
							...
							'heading'    => esc_html__( 'Product Info Color', 'onea' ),
						...
						array(
							...
							'heading'     => esc_html__( 'Category Tag', 'onea' ),
							'value'       => array_flip( onea_elated_get_title_tag( true ) ),
							'description' => esc_html__( 'Set category tag for product category element', 'onea' ),
						...
						array(
							...
							'heading'    => esc_html__( 'Add To Cart Skin', 'onea' ),
							'value'      => array(
								esc_html__( 'Default', 'onea' ) => '',
								esc_html__( 'White', 'onea' )   => 'white',
						...
						array(
							...
							'heading'    => esc_html__( 'Add To Cart Size', 'onea' ),
							'value'      => array(
								esc_html__( 'Default', 'onea' ) => '',
							...
							'dependency' => array( 'element' => 'product_info_type', 'value'   => array( 'onea' ) ),
							'group'      => esc_html__( 'Product Info Style', 'onea' )
						)
					)
				)
	...
	public function getItemTitleHtml( $params ) {
		...
		if ( ! empty( $title ) ) {
			$html = '<' . esc_attr( $title_tag ) . ' itemprop="name" class="eltdf-pi-title entry-title" ' . onea_elated_get_inline_style( $product_info_style ) . '>';
			...
			$html .= '<div class="eltdf-pi-category">';
				foreach ( $categories as $cat ) {
					if ( ! empty( $params['category_tag'] ) ) {
						$html .= '<' . esc_attr( $params['category_tag'] ) . ' ' . onea_elated_get_inline_style( $product_info_style ) . '>';
						...
						$html .= '<a itemprop="url" class="eltdf-pi-category-item" href="' . esc_url( get_term_link( $cat->term_id ) ) . '" ' . onea_elated_get_inline_style( $product_info_style ) . '>' . esc_html( $cat->name ) . '</a>';
					}
				}
			$html .= '</div>';
		}
	...
	public function getItemExcerptHtml( $params ) {
		...
		if ( ! empty( $excerpt ) ) {
			$html = '<div class="eltdf-pi-excerpt"><p itemprop="description" class="eltdf-pi-excerpt-item" ' . onea_elated_get_inline_style( $product_info_style ) . '>' . esc_html( $excerpt ) . '</p></div>';
		}
	...
	public function getItemPriceHtml( $params ) {
		...
		if ( $price_html = $product->get_price_html() ) {
			$html = '<div class="eltdf-pi-price" ' . onea_elated_get_inline_style( $product_info_style ) . '>' . $price_html . '</div>';
		}
	...
	public function getItemRatingHtml( $params ) {
		...
		if ( get_option( 'woocommerce_enable_review_rating' ) !== 'no' ) {
			...
			$html = '<div class="eltdf-pi-rating" title="' . sprintf( esc_attr__( "Rated %s out of 5", "onea" ), $average ) . '" ' . onea_elated_get_inline_style( $product_info_style ) . '><span style="width:' . ( ( $average / 5 ) * 100 ) . '%"></span></div>';
		}
	...
	public function getItemAddToCartHtml( $params ) {
		...
		$html .= apply_filters( 'onea_elated_filter_product_info_add_to_cart_link',
	...
	public function getItemTagHtml( $params ) {
		...
		if ( ! empty( $tags ) ) {
			$html = '<div class="eltdf-pi-tag">';
				foreach ( $tags as $tag ) {
					$html .= '<a itemprop="url" class="eltdf-pi-tag-item" href="' . esc_url( get_term_link( $tag->term_id ) ) . '" ' . onea_elated_get_inline_style( $product_info_style ) . '>' . esc_html( $tag->name ) . '</a>';
				}
			$html .= '</div>';
		}
	...
	public function getItemAuthorHtml( $params ) {
		...
		if ( ! empty( $author ) ) {
			$html = '<div class="eltdf-pi-author" ' . onea_elated_get_inline_style( $product_info_style ) . '>' . esc_html( $author ) . '</div>';
		}
	...
	public function getItemDateHtml( $params ) {
		...
		if ( ! empty( $date ) ) {
			$html = '<div class="eltdf-pi-date" ' . onea_elated_get_inline_style( $product_info_style ) . '>' . esc_html( $date ) . '</div>';
		}
