...
class OneaElatedClassLike {
	private static $instance;
	
	private function __construct() {
		add_action( 'wp_ajax_onea_elated_like', array( $this, 'ajax' ) );
		add_action( 'wp_ajax_nopriv_onea_elated_like', array( $this, 'ajax' ) );
	}
	...
	public function add_like() {
		...
		$title       = esc_html__( 'Like this', 'onea' );
		
		if ( isset( $_COOKIE[ 'eltdf-like_' . $post->ID ] ) ) {
			$class = 'eltdf-like liked';
			$title = esc_html__( 'You already like this!', 'onea' );
		}
...
if ( ! function_exists( 'onea_elated_create_like' ) ) {
	function onea_elated_create_like() {
		OneaElatedClassLike::get_instance();
	}
	
	add_action( 'after_setup_theme', 'onea_elated_create_like' );
}