...
include_once 'onea-twitter-helper.php';

/**
 * Class OneaTwitterApi
 */
class OneaTwitterApi {
	/**
	...
	const REQUEST_TOKEN_FIELD = 'onea_twitter_request_token';
	const REQUEST_TOKEN_SECRET_FIELD = 'onea_twitter_request_token_secret';
	const ACCESS_TOKEN_FIELD = 'onea_twitter_access_token';
	const ACCESS_TOKEN_SECRET_FIELD = 'onea_twitter_access_token_secret';
	const AUTHORIZE_TOKEN_FIELD = 'onea_twitter_authorize_token';
	const AUTHORIZE_VERIFIER_FIELD = 'onea_twitter_authorize_verifier';
	const USER_ID_FIELD = 'onea_twitter_user_id';
	const USER_SCREEN_NAME_FIELD = 'onea_twitter_screen_name';
	...
	public function __construct() {
		...
		$this->helper          = new OneaTwitterHelper();
	}
	
	
	/**
	 * @return OneaTwitterApi
	 */
	 ...
	 *
	 * @see OneaTwitterApi::generateSignatureBase()
	 */
	...
	public function obtainRequestToken() {
		...
		if ( is_wp_error( $response ) ) {
			...
			$responseObj->message = esc_html__( 'Internal WP error', 'onea-twitter-feed' );
		} else {
			...
			if ( ! empty( $responseBody ) ) {
				...
				if ( ( is_array( $responseParsed ) && count( $responseParsed ) ) &&
					...
					if ( ! empty( $responseObj->redirectUrl ) ) {
						$responseObj->status  = false;
						$responseObj->message = esc_html__( 'Redirect URL couldn\t not be generated', 'onea-twitter-feed' );
					} else {
					...
					$responseObj->status  = false;
					$responseObj->message = esc_html__( 'Couldn\'t connect with Twitter API', 'onea-twitter-feed' );
				}
			}
		}
		...
		if ( ! empty( $authorizeVerifier ) && ! empty( $authorizeToken ) ) {
			...
			if ( is_wp_error( $response ) ) {
				...
				if ( is_array( $responseParsed ) && count( $responseParsed )
					...
					$responseObj->status  = true;
					$responseObj->message = esc_html__( 'Access token obtained', 'onea-twitter-feed' );
				}
			}
		} else {
			$responseObj->status  = false;
			$responseObj->message = esc_html__( 'Authorize token and it\'s secret were not obtainer', 'onea-twitter-feed' );
		}
	...
	public function fetchTweets( $userId = '', $count = '', $transient = array() ) {
		...
		if ( ! $this->transientExists( $transient ) ) {
			if ( $userId && $accessToken ) {
				...
				if ( is_wp_error( $response ) ) {
					...
					if ( isset( $response['response']['code'] ) && $response['response']['code'] == 200 ) {
						...
						$responseObj->status  = false;
						$responseObj->message = esc_html__( 'Couldn\'t connect with Twitter', 'onea-twitter-feed' );
					}
				}
			} else {
				$responseObj->status  = false;
				$responseObj->message = esc_html__( 'It seams like you haven\t connected with your Twitter account', 'onea-twitter-feed' );
			}
		} else {
			...
			if ( ! empty( $transientContent ) ) {
				...
				$responseObj->status  = false;
				$responseObj->message = esc_html__( 'Couldn\'t retreive content from database', 'onea-twitter-feed' );
			}
		}
