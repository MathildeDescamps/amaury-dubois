    ...
    <div class="eltdf-tl-wrapper eltdf-outer-space">
            ...
            <?php if ( is_array( $response->data ) && count( $response->data ) ) { ?>
                <ul class="eltdf-twitter-list">
                    <?php foreach ( $response->data as $tweet ) { ?>
                        <?php
                            $params['tweet'] = $tweet;
                            echo onea_twitter_get_shortcode_module_template_part('templates/item', 'twitter-list', '', $params);
                        ?>
                    <?php } ?>
                </ul>
            <?php } else { ?>
        ...
        <?php } else { ?>
            <div class="eltdf-twitter-not-connected">
                <?php esc_html_e( 'It seams that you haven\'t connected with your Twitter account', 'onea-twitter-feed' ); ?>
            </div>
        <?php } ?>
    </div>
</div>