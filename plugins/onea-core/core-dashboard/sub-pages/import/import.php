...
class OneaCoreImport {
	...
	public function set_import_url() {
		$params = OneaCoreDashboard::get_instance()->get_import_params();
	...
	public function import_action() {
				...
				if ( wp_verify_nonce( $_POST['options']['nonce'], 'eltdf_cd_import_nonce' ) ) {
					...
					switch ($_POST['options']['action']):
						...
						case 'menu-settings':
							...
							break;
						case 'rev-slider':
							if (onea_core_is_revolution_slider_installed()) {
								$this->rev_slider_import($demo);
							} else {
								$this->set_status('success');
								$this->set_data('type', 'options');
								$this->set_message(esc_html__('Revolution Slider isn\'t installed', 'onea-core'));
							}
							break;
						case 'content':
							...
							if($content_start) {
								if (!OneaCoreDashboard::get_instance()->check_purchase_code($_POST['options']['demo'])) {
									onea_core_ajax_status('error', esc_html__('Please don\'t try to hack me. Purchase code registered is not valid', 'onea-core'));
									exit;
								}
							}
			...
			onea_core_ajax_status($this->get_status(), $this->get_message(), $this->get_data());
		}
		wp_die();
	}
	...
	function file_content( $path ) {
		...
		if ( '200' != wp_remote_retrieve_response_code( $response ) ) {
			$this->set_message($response["response"]['message'] . ' ' . esc_html__('Please contact support', 'onea-core'));
	...
	public function import_widgets($demo) {
		...
		if($widgets_content) {
			foreach ((array)$widgets_content['widgets'] as $onea_widget_id => $onea_widget_data) {
				update_option('widget_' . $onea_widget_id, $onea_widget_data);
			}
			$ws = $this->import_sidebars_widgets($widgets);
			if($ws) {
				$this->set_message(esc_html__('Widgets are set for proper sidebar', 'onea-core'));
				$this->set_data('type', 'options');
				$this->set_status('success');
			}
		}
	...
	public function import_sidebars_widgets( $file ) {
		$onea_sidebars = get_option( "sidebars_widgets" );
		unset( $onea_sidebars['array_version'] );
		$data = $this->unserialized_content( $file );

		if ( $data && is_array( $data['sidebars'] ) ) {
			$onea_sidebars = array_merge( (array) $onea_sidebars, (array) $data['sidebars'] );
			unset( $onea_sidebars['wp_inactive_widgets'] );
			$onea_sidebars                  = array_merge( array( 'wp_inactive_widgets' => array() ), $onea_sidebars );
			$onea_sidebars['array_version'] = 2;
			wp_set_sidebars_widgets( $onea_sidebars );
			return true;
		} else {
			return false;
		}
	...
	public function import_options( $file ) {
		...
		$current_options = get_option(ONEA_CORE_OPTIONS_NAME);
		if($options){
			if($current_options != $options) {
				$result = update_option(ONEA_CORE_OPTIONS_NAME, $options);
				if ($result) {
					$this->update_options_after_import($file);
					$this->set_status('success');
					$this->set_data('type', 'options');
					$this->set_message(esc_html__('Options imported successfully', 'onea-core'));

					$this->update_options_after_import($file);

				} else {
					$this->set_status('error');
					$this->set_message(esc_html__('Problem occurred during options import', 'onea-core'));
				}
			} else {
				$this->set_status('success');
				$this->set_data('type', 'options');
				$this->set_message(esc_html__('Options are already imported', 'onea-core'));
			}
		}
	...
	public function import_settings_pages( $file ) {
		...
		$new_ids = get_transient( '_onea_core_imported_posts' );
		$fields_status = true;
		...
		if($pages) {
			...
			if (!$fields_status) {
				$this->set_status('error');
				$this->set_message(esc_html__('Problem occurred during settings pages import', 'onea-core'));
			} else {
				$this->set_status('success');
				$this->set_data('type', 'options');
				$this->set_message(esc_html__('Settings pages imported successfully', 'onea-core'));
			}
		} else {
			$this->set_status('error');
			$this->set_message(esc_html__('File doesn\'t exist', 'onea-core'));
		}
	}
	...
	public function import_menu_settings( $file ) {
		...
		if($menus_data) {
			...
			$this->set_status('success');
			$this->set_data('type', 'options');
			$this->set_message( esc_html__( 'Menus set for proper locations', 'onea-core' ) );
		} else {
			$this->set_status('error');
			$this->set_message( esc_html__( 'Problem during menus location set', 'onea-core' ) );
		}
	}
	...
	public function import_content( $file, $xml, $attachments, $post_id) {
		ob_start();
		require_once( ONEA_CORE_ABS_PATH . '/core-dashboard/sub-pages/import/wordpress-importer.php' );
...
//		}
//
		if(onea_core_is_woocommerce_installed()) {
		...
		$onea_import = new WP_Import();
		set_time_limit( 0 );

		$onea_import->fetch_attachments = $attachments;
		$returned_value                  = $onea_import->import( $file . $xml );
		...
		if ( is_wp_error( $returned_value ) ) {
			...
			$this->set_message( esc_html__( 'An error occurred during content import', 'onea-core' ) );
		} else {
			$this->set_status('success');
			$this->set_data('type', 'content');
			$this->set_data('posts', $this->imported_posts);
			$this->set_message( esc_html__( 'File imported successfully', 'onea-core' ) . ' ' . $xml );
	...
	public function rev_sliders() {
		$rev_sldiers = array(
            ...
            'blog-pinterest.zip',
            'coming-soon.zip',
            'content-slider-1.zip',
            'cosmetic-store-slider.zip',
            'furniture-home-slider.zip',
            'kids-store-slider.zip',
            'landing-content-1-slider.zip',
            'landing-content-2-slider.zip',
            'landing-end.zip',
            'landing-slider-1.zip',
            'main-home-slider.zip',
            'mens-fashion-slider.zip',
            'organic-home-slider.zip',
            'payments.zip',
            'product-showcase-slider.zip',
            'shop-grid-slider.zip',
            'sunglasses-store-slider.zip',
            'sunglasses-store-slider-2.zip',
            'sunglasses-store-slider-3.zip',
            'vertical-slider.zip',
            'womens-fashion-slider.zip'
		);
		...
		if($files_created) {
			...
			foreach ($rev_sliders as $rev_slider) {
				...
				if (!$rev_results['success']) {
					$this->set_status('error');
					$this->set_message(esc_html__('Error while importing rev sliders', 'onea-core'));
					exit;
				}
			}
			$this->set_status('success');
			$this->set_data('type', 'options');
			$this->set_message(esc_html__('Rev sliders imported successfully', 'onea-core'));
		} else {
			$this->set_status('error');
			$this->set_data('type', 'options');
			$this->set_message(esc_html__('Files don\'t exist', 'onea-core'));
		}
	}
	...
	function update_options_after_import( $folder ) {
		...
		foreach ( $demo_urls as $demo_url ) {
			$global_options    = get_option( ONEA_CORE_OPTIONS_NAME );
			$new_global_values = str_replace( $demo_url, $url, $global_options );

			update_option( ONEA_CORE_OPTIONS_NAME, $new_global_values );
		}
	}
	...
	function get_noimage_url( $origin_img_url ) {
		...
		return ONEA_CORE_ASSETS_URL_PATH . '/img/' . $noimage_fname;
	}
	...
	public function populate_single_pages() {
		...
		if ( isset( $_POST ) && !empty( $_POST ) && !empty($_POST['options']['demo']) ) {
			if ( wp_verify_nonce( $_POST['options']['nonce'], 'eltdf_cd_import_nonce' ) ) {
				...
				$html = onea_core_get_module_template_part('core-dashboard/sub-pages/import', 'pages-list', '', array('pages' => $pages));

				if($pages){
					onea_core_ajax_status( 'success', '', $html);
				} else {
					onea_core_ajax_status( 'error', esc_html__( 'Pages don\'t exist', 'onea-core' ), '');
				}
			}
		}
	...
	public function is_ready_to_import() {
		$info = OneaCoreSystemInfoPage::get_instance()->get_system_info();
