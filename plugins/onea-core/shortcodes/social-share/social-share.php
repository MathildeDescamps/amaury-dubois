<?php
namespace OneaCore\CPT\Shortcodes\SocialShare;
...
class SocialShare implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if ( function_exists( 'vc_map' ) ) {
			vc_map(
				array(
					'name'                      => esc_html__( 'Social Share', 'onea-core' ),
					'base'                      => $this->getBase(),
					...
					'params'                    => array(
						array(
							...
							'value'      => array(
								esc_html__( 'List', 'onea-core' )     => 'list',
								esc_html__( 'Dropdown', 'onea-core' ) => 'dropdown',
								esc_html__( 'Text', 'onea-core' )     => 'text'
							)
						),
						array(
							'type'       => 'dropdown',
							'param_name' => 'icon_type',
							'heading'    => esc_html__( 'Icons Type', 'onea-core' ),
							'value'      => array(
								esc_html__( 'Font Awesome', 'onea-core' ) => 'font-awesome',
								esc_html__( 'Font Elegant', 'onea-core' ) => 'font-elegant'
							),
						...
						array(
							'type'       => 'textfield',
							'param_name' => 'title',
							'heading'    => esc_html__( 'Social Share Title', 'onea-core' )
						)
					)
				)
			);
	...
	public function render( $atts, $content = null ) {
		...
		$params['enable_social_share'] = ( onea_elated_options()->getOptionValue( 'enable_social_share' ) == 'yes' ) ? true : false;
		
		//Is social share enabled for post type
		$post_type         = str_replace( '-', '_', get_post_type() );
		$params['enabled'] = ( onea_elated_options()->getOptionValue( 'enable_social_share_on_' . $post_type ) == 'yes' ) ? true : false;
		...
		if ( $params['enable_social_share'] ) {
			if ( $params['enabled'] ) {
				$html .= onea_core_get_shortcode_module_template_part( 'templates/' . $params['type'], 'social-share', '', $params );
			}
		}
	...
	private function getSocialNetworksParams( $params ) {
		...
		foreach ( $this->socialNetworks as $net ) {
			...
			if ( onea_elated_options()->getOptionValue( 'enable_' . $net . '_share' ) == 'yes' ) {
				...
				$params['custom_icon'] = ( onea_elated_options()->getOptionValue( $net . '_icon' ) ) ? onea_elated_options()->getOptionValue( $net . '_icon' ) : '';
				
				$html = onea_core_get_shortcode_module_template_part( 'templates/parts/network', 'social-share', '', $params );
			}
    ...
    private function getSocialNetworkShareLink($net, $image) {
        switch ($net) {
            ...
            case 'twitter':
                ...
                $twitter_via = (onea_elated_options()->getOptionValue('twitter_via') !== '') ? ' via ' . onea_elated_options()->getOptionValue('twitter_via') . ' ' : '';
	            $link = 'window.open(\'https://twitter.com/intent/tweet?text=' . urlencode(onea_elated_the_excerpt_max_charlength($count_char) . $twitter_via) . get_permalink() . '\', \'popupwindow\', \'scrollbars=yes,width=800,height=400\');';
	            break;
            case 'linkedin':
	...
	private function getSocialNetworkText( $net ) {
		switch ( $net ) {
			case 'facebook':
				$text = esc_html__( 'fb', 'onea-core');
				break;
			case 'twitter':
				$text = esc_html__( 'tw', 'onea-core');
				break;
			case 'linkedin':
				$text = esc_html__( 'lnkd', 'onea-core');
				break;
			case 'tumblr':
				$text = esc_html__( 'tmb', 'onea-core');
				break;
			case 'pinterest':
				$text = esc_html__( 'pin', 'onea-core');
				break;
			case 'vk':
				$text = esc_html__( 'vk', 'onea-core');
				break;
			default:
				$text = '';
		}
