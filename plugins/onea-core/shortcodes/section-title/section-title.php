<?php
namespace OneaCore\CPT\Shortcodes\SectionTitle;
...
class SectionTitle implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if ( function_exists( 'vc_map' ) ) {
			vc_map(
				array(
					'name'                      => esc_html__( 'Section Title', 'onea-core' ),
					'base'                      => $this->base,
					...
					'params'                    => array(
						...
						array(
							...
							'value'       => array(
								...
								esc_html__( 'Two Columns', 'onea-core' ) => 'two-columns'
							),
							'save_always' => true
						),
						array(
							...
							'heading'     => esc_html__( 'Title - Text Position', 'onea-core' ),
							'value'       => array(
								esc_html__( 'Title Left - Text Right', 'onea-core' ) => 'title-left',
								esc_html__( 'Title Right - Text Left', 'onea-core' ) => 'title-right'
							),
							'save_always' => true,
						...
						array(
							...
							'heading'     => esc_html__( 'Space Between Columns', 'onea-core' ),
							'value'       => array(
								esc_html__( 'Normal', 'onea-core' ) => 'normal',
								esc_html__( 'Small', 'onea-core' )  => 'small',
								esc_html__( 'Tiny', 'onea-core' )   => 'tiny'
							),
							'save_always' => true,
							'dependency'  => array( 'element' => 'type', 'value' => array( 'two-columns' ) )
						),
						array(
							'type'        => 'dropdown',
							'param_name'  => 'position',
							'heading'     => esc_html__( 'Horizontal Position', 'onea-core' ),
							'value'       => array(
						...
						array(
							...
							'heading'    => esc_html__( 'Holder Side Padding (px or %)', 'onea-core' )
						),
						array(
							'type'        => 'textfield',
							'param_name'  => 'subtitle',
							'heading'     => esc_html__( 'SubTitle Above', 'onea-core' ),
							'admin_label' => true
						),
						array(
							'type'        => 'textfield',
						...
						array(
							...
							'heading'     => esc_html__( 'Words with Bold Font Weight', 'onea-core' ),
							'description' => esc_html__( 'Enter the positions of the words you would like to display in a "bold" font weight. Separate the positions with commas (e.g. if you would like the first, second, and third word to have a light font weight, you would enter "1,2,3")', 'onea-core' ),
							'dependency'  => array( 'element' => 'title', 'not_empty' => true ),
							'group'       => esc_html__( 'Title Style', 'onea-core' )
						),
						array(
							'type'        => 'textfield',
							'param_name'  => 'title_background_words',
							'heading'     => esc_html__( 'Words with Background', 'onea-core' ),
							'description' => esc_html__( 'Enter the positions of the words you would like to have a background. Separate the positions with commas (e.g. if you would like the first, second, and third word to have a light font weight, you would enter "1,2,3")', 'onea-core' ),
							'dependency'  => array( 'element' => 'title', 'not_empty' => true ),
							'group'       => esc_html__( 'Title Style', 'onea-core' )
						),
						array(
							...
							'heading'     => esc_html__( 'Words with Light Font Weight', 'onea-core' ),
							'description' => esc_html__( 'Enter the positions of the words you would like to display in a "light" font weight. Separate the positions with commas (e.g. if you would like the first, third, and fourth word to have a light font weight, you would enter "1,3,4")', 'onea-core' ),
							'dependency'  => array( 'element' => 'title', 'not_empty' => true ),
							'group'       => esc_html__( 'Title Style', 'onea-core' )
						),
						array(
						...
						array(
							...
							'group'       => esc_html__( 'Text Style', 'onea-core' )
						),
						array(
							'type'       => 'colorpicker',
						...
						array(
							...
							'heading'    => esc_html__( 'Text Font Size (px)', 'onea-core' ),
							'dependency' => array( 'element' => 'text', 'not_empty' => true ),
							'group'      => esc_html__( 'Text Style', 'onea-core' )
						),
						array(
							...
							'heading'    => esc_html__( 'Text Line Height (px)', 'onea-core' ),
							'dependency' => array( 'element' => 'text', 'not_empty' => true ),
							'group'      => esc_html__( 'Text Style', 'onea-core' )
						),
						array(
							...
							'heading'     => esc_html__( 'Text Font Weight', 'onea-core' ),
	...
	public function render( $atts, $content = null ) {
		...
		$html = onea_core_get_shortcode_module_template_part( 'templates/section-title', 'section-title', '', $params );
		...
		if ( ! empty( $title ) ) {
			...
			if ( ! empty( $title_background_words ) ) {
				foreach ( $background_words as $value ) {
					if ( ! empty( $split_title[ $value - 1 ] ) ) {
						$split_title[ $value - 1 ] = '<span class="eltdf-st-title-background">' . $split_title[ $value - 1 ] . '</span>';
					}
				}
			}
	...
	private function getTextStyles( $params ) {
		...
		if ( ! empty( $params['text_line_height'] ) ) {
			$styles[] = 'line-height: ' . onea_elated_filter_px( $params['text_line_height'] ) . 'px';
		}
		...
		if ( $params['text_margin'] !== '' ) {
			$styles[] = 'margin-top: ' . onea_elated_filter_px( $params['text_margin'] ) . 'px';
		}
