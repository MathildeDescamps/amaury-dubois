<?php
namespace OneaCore\CPT\Shortcodes\Button;
...
class Button implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if ( function_exists( 'vc_map' ) ) {
			vc_map(
				array(
					'name'                      => esc_html__( 'Button', 'onea-core' ),
					'base'                      => $this->base,
					...
					'params'                    => array_merge(
						...
						onea_elated_icon_collections()->getVCParamsArray( array(), '', true ),
						array(
							array(
								'type'       => 'colorpicker',
								'param_name' => 'color',
								'heading'    => esc_html__( 'Color', 'onea-core' ),
								'group'      => esc_html__( 'Design Options', 'onea-core' )
							),
							array(
								'type'       => 'colorpicker',
								'param_name' => 'hover_color',
								'heading'    => esc_html__( 'Hover Color', 'onea-core' ),
								'group'      => esc_html__( 'Design Options', 'onea-core' )
							),
							array(
								'type'       => 'colorpicker',
							...
							array(
								...
								'heading'    => esc_html__( 'Font Size (px)', 'onea-core' ),
								'group'      => esc_html__( 'Design Options', 'onea-core' )
							),
							array(
								'type'        => 'dropdown',
							...
							array(
								'type'        => 'textfield',
								'param_name'  => 'padding',
								'heading'     => esc_html__( 'Button Padding', 'onea-core' ),
								'description' => esc_html__( 'Insert padding in format: top right bottom left (e.g. 10px 5px 10px 5px)', 'onea-core' ),
								'dependency'  => array( 'element' => 'type', 'value' => array( 'solid', 'outline' ) ),
								'group'       => esc_html__( 'Design Options', 'onea-core' )
							)
						)
	...
	public function render( $atts, $content = null ) {
		...
		return onea_core_get_shortcode_module_template_part( 'templates/' . $params['html_type'], 'button', '', $params );
	}
	...
	private function getButtonStyles( $params ) {
		...
		if ( ! empty( $params['font_size'] ) ) {
			$styles[] = 'font-size: ' . onea_elated_filter_px( $params['font_size'] ) . 'px';
		}
