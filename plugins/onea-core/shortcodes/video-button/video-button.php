...
namespace OneaCore\CPT\Shortcodes\VideoButton;
...
class VideoButton implements Lib\ShortcodeInterface {
	...
	public function vcMap() {
		if ( function_exists( 'vc_map' ) ) {
			vc_map(
				array(
					'name'                      => esc_html__( 'Video Button', 'onea-core' ),
					'base'                      => $this->getBase(),
					...
					'params'                    => array(
						...
						array(
							...
							'heading'    => esc_html__( 'Video Link', 'onea-core' )
						),
						array(
							'type'        => 'attach_image',
							'param_name'  => 'video_image',
							'heading'     => esc_html__( 'Video Image', 'onea-core' ),
							'description' => esc_html__( 'Select image from media library', 'onea-core' )
						),
						array(
							'type'       => 'colorpicker',
							'param_name' => 'play_button_color',
							'heading'    => esc_html__( 'Play Button Color', 'onea-core' )
						),
						array(
							'type'       => 'textfield',
							'param_name' => 'play_button_size',
							'heading'    => esc_html__( 'Play Button Size (px)', 'onea-core' )
						),
						array(
							'type'        => 'attach_image',
							'param_name'  => 'play_button_image',
							'heading'     => esc_html__( 'Play Button Custom Image', 'onea-core' ),
							'description' => esc_html__( 'Select image from media library. If you use this field then play button color and button size options will not work', 'onea-core' )
						),
						array(
							'type'        => 'attach_image',
							'param_name'  => 'play_button_hover_image',
							'heading'     => esc_html__( 'Play Button Custom Hover Image', 'onea-core' ),
							'description' => esc_html__( 'Select image from media library. If you use this field then play button color and button size options will not work', 'onea-core' )
						)
					)
				)
	...
	public function render( $atts, $content = null ) {
		...
		$html = onea_core_get_shortcode_module_template_part( 'templates/video-button', 'video-button', '', $params );
	...
	private function getPlayButtonStyles( $params ) {
		...
		if ( ! empty( $params['play_button_size'] ) ) {
			$styles[] = 'font-size: ' . onea_elated_filter_px( $params['play_button_size'] ) . 'px';
		}
