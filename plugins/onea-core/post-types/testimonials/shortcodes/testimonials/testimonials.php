...
namespace OneaCore\CPT\Shortcodes\Testimonials;
...
class Testimonials implements Lib\ShortcodeInterface {
					...
					'params'                    => array(
						array(
							...
							'value'       => array(
								...
								esc_html__( 'Boxed', 'onea-core' )               => 'boxed',
								esc_html__( 'Image Pagination', 'onea-core' )    => 'image-pagination',
								esc_html__( 'Boxed Text', 'onea-core' )          => 'boxed-text',
						...
						array(
							'type'       => 'textfield',
							'param_name' => 'number',
							'heading'    => esc_html__( 'Number of Testimonials', 'onea-core' )
						),
						array(
							'type'        => 'autocomplete',
							'param_name'  => 'category',
						...
						array(
							...
							'heading'    => esc_html__( 'Content Box Color', 'onea-core' ),
	...
	public function render( $atts, $content = null ) {
        ...
        return onea_core_get_cpt_shortcode_module_template_part( 'testimonials', 'testimonials', 'testimonials-' . $params['type'], '', $params );
	...
	public function testimonialsCategoryAutocompleteSuggester( $query ) {
		...
		if ( is_array( $post_meta_infos ) && ! empty( $post_meta_infos ) ) {
			foreach ( $post_meta_infos as $value ) {
				...
				$data['label'] = ( ( strlen( $value['testimonials_category_title'] ) > 0 ) ? esc_html__( 'Category', 'onea-core' ) . ': ' . $value['testimonials_category_title'] : '' );
				$results[]     = $data;
			}
		}
		...
		if ( ! empty( $query ) ) {
			...
			if ( is_object( $testimonials_category ) ) {
				...
				if ( ! empty( $testimonials_category_title ) ) {
					$testimonials_category_title_display = esc_html__( 'Category', 'onea-core' ) . ': ' . $testimonials_category_title;
				}
