<?php if($filter == 'yes') {
	...
	?>
	<div class="eltdf-pl-filter-holder" <?php onea_elated_inline_style($filter_holder_styles); ?>>
		<div class="eltdf-plf-inner">
			<?php
			if(is_array($filter_categories) && count($filter_categories)){ ?>
				<ul <?php onea_elated_inline_style($filter_styles); ?>>
					<li class="eltdf-pl-filter" data-filter="">
						<span><?php esc_html_e('Show all', 'onea-core')?></span>
					</li>
