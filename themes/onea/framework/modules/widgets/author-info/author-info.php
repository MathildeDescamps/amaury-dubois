...
if (class_exists('OneaCoreClassWidget')) {
	class OneaElatedClassAuthorInfoWidget extends OneaCoreClassWidget
	{
		public function __construct()
		{
			parent::__construct(
				'eltdf_author_info_widget',
				esc_html__('Onea Author Info Widget', 'onea'),
				array('description' => esc_html__('Add author info element to widget areas', 'onea'))
			);
		...
		protected function setParams()
		{
			$this->params = array(
				...
				array(
					...
					'title' => esc_html__('Author Username', 'onea')
				)
			);
		}
		...
		public function widget($args, $instance)
		{
			...
			<div class="widget eltdf-author-info-widget <?php echo esc_attr($extra_class); ?>" <?php echo onea_elated_get_inline_style($widget_styles); ?>>
				<div class="eltdf-aiw-inner">
					<a itemprop="url" class="eltdf-aiw-image"
					   href="<?php echo esc_url(get_author_posts_url($authorID)); ?>">
						<?php echo onea_elated_kses_img(get_avatar($authorID, 275)); ?>
					</a>
					<?php if ($author_info !== "") { ?>
						<h4 class="eltdf-aiw-title"><?php esc_html_e('About Author', 'onea'); ?></h4>
