...
if ( class_exists( 'OneaCoreClassWidget' ) ) {
	class OneaElatedClassIconWidget extends OneaCoreClassWidget
	{
		public function __construct()
		{
			parent::__construct(
				'eltdf_icon_widget',
				esc_html__('Onea Icon Widget', 'onea'),
				array('description' => esc_html__('Add icons to widget areas', 'onea'))
			);
		...
		protected function setParams()
		{
			$this->params = array_merge(
				onea_elated_icon_collections()->getIconWidgetParamsArray(),
				array(
					array(
						'type' => 'textfield',
						'name' => 'icon_text',
						'title' => esc_html__('Icon Text', 'onea')
					),
					array(
						'type' => 'textfield',
						'name' => 'link',
					...
					array(
						'type' => 'textfield',
						'name' => 'text_size',
						'title' => esc_html__('Text Size (px)', 'onea')
					),
					array(
						'type' => 'colorpicker',
						'name' => 'text_color',
						'title' => esc_html__('Text Color', 'onea')
					),
					array(
						'type' => 'textfield',
						'name' => 'margin',
		...
		public function widget($args, $instance)
		{
			...
			$data = array();
			$data[] = !empty($instance['icon_hover_color']) ? onea_elated_get_inline_attr($instance['icon_hover_color'], 'data-hover-color') : '';
			$data = implode(' ', $data);
			...
			if (!empty($instance['text_size'])) {
				$text_styles[] = 'font-size: ' . onea_elated_filter_px($instance['text_size']) . 'px';
			}
			...
			if (!empty($instance['icon_pack'])) {
				...
				if (!empty($icon_class)) {
					...
					$icon_holder_html = '<span class="eltdf-icon-element ' . esc_attr($icon_class) . '" ' . onea_elated_get_inline_style($icon_styles) . '></span>';
				}
			}
			...
			if (!empty($instance['icon_text'])) {
				$icon_text_html = '<span class="eltdf-icon-text ' . esc_attr($icon_text_class) . '" ' . onea_elated_get_inline_style($text_styles) . '>' . esc_html($instance['icon_text']) . '</span>';
			}
			?>

			<a <?php onea_elated_class_attribute($holder_classes); ?> <?php echo wp_kses_post($data); ?>
					href="<?php echo esc_url($link); ?>"
					target="<?php echo esc_attr($target); ?>" <?php echo onea_elated_get_inline_style($holder_styles); ?>>
