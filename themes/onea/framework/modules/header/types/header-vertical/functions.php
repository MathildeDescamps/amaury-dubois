...
if ( ! function_exists( 'onea_elated_register_header_vertical_type' ) ) {
	/**
	 * This function is used to register header type class for header factory file
	 */
	function onea_elated_register_header_vertical_type( $header_types ) {
		$header_type = array(
			'header-vertical' => 'OneaElatedNamespace\Modules\Header\Types\HeaderVertical'
		);
...
if ( ! function_exists( 'onea_elated_init_register_header_vertical_type' ) ) {
	/**
	 * This function is used to wait header-function.php file to init header object and then to init hook registration function above
	 */
	function onea_elated_init_register_header_vertical_type() {
		add_filter( 'onea_elated_filter_register_header_type_class', 'onea_elated_register_header_vertical_type' );
	}
	
	add_action( 'onea_elated_action_before_header_function_init', 'onea_elated_init_register_header_vertical_type' );
}

if ( ! function_exists( 'onea_elated_include_header_vertical_menu' ) ) {
	/**
	 * Registers additional menu navigation for theme
	 */
	function onea_elated_include_header_vertical_menu( $menus ) {
		$menus['vertical-navigation'] = esc_html__( 'Vertical Navigation', 'onea' );
		
		return $menus;
	}
	
	if ( onea_elated_check_is_header_type_enabled( 'header-vertical' ) ) {
		add_filter( 'onea_elated_filter_register_headers_menu', 'onea_elated_include_header_vertical_menu' );
	}
}

if ( ! function_exists( 'onea_elated_get_header_vertical_main_menu' ) ) {
	/**
	 * Loads vertical menu HTML
	 */
	function onea_elated_get_header_vertical_main_menu() {
		$menu_opening = onea_elated_options()->getOptionValue('vertical_menu_dropdown_opening');
		
		$params = array(
			'opening_class' => 'eltdf-vertical-dropdown-'. ( $menu_opening !== '' ? $menu_opening : 'below' )
		);

		onea_elated_get_module_template_part( 'templates/vertical-navigation', 'header/types/header-vertical', '', $params );
	}
}

if ( ! function_exists( 'onea_elated_vertical_header_holder_class' ) ) {
	/**
	 * Return holder class
	 */
	function onea_elated_vertical_header_holder_class() {
...
if ( ! function_exists( 'onea_elated_header_vertical_per_page_custom_styles' ) ) {
	/**
	 * Return header per page styles
	 */
	function onea_elated_header_vertical_per_page_custom_styles( $style, $class_prefix, $page_id ) {
	...
	add_filter( 'onea_elated_filter_add_header_page_custom_style', 'onea_elated_header_vertical_per_page_custom_styles', 10, 3 );
}