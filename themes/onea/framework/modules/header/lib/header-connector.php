...
namespace OneaElatedNamespace\Modules\Header\Lib;

/**
 * Class OneaElatedClassHeaderConnector
 *
...
class OneaElatedClassHeaderConnector {
	...
	public function connect( $config = array() ) {
		do_action( 'onea_elated_action_pre_header_connect' );
		...
		if ( ! empty( $config['affect_content'] ) ) {
			add_filter( 'onea_elated_filter_content_elem_style_attr', array( $this, 'contentMarginFilter' ) );
		}
		
		if ( ! empty( $config['affect_title'] ) ) {
			add_filter( 'onea_elated_filter_title_content_padding', array( $this, 'titlePaddingFilter' ) );
		}
		
		do_action( 'onea_elated_action_after_header_connect' );
	}
	 ...
	 *
	 * Hooks to onea_elated_filter_title_content_padding filter
	 *
	 * @return int
	 */
