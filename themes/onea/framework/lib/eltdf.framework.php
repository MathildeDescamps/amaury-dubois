...
/*
   Class: OneaElatedClassFramework
...
class OneaElatedClassFramework {
	...
	private function __construct() {
		$this->eltdOptions         = OneaElatedClassOptions::get_instance();
		$this->eltdMetaBoxes       = OneaElatedClassMetaBoxes::get_instance();
		$this->eltdTaxonomyOptions = OneaElatedClassTaxonomyOptions::get_instance();
		$this->eltdUserOptions     = OneaElatedClassUserOptions::get_instance();
		$this->eltdDashboardOptions = OneaElatedClassDashboardOptions::get_instance();
	}
	...
	public function setSkin( OneaElatedClassSkinAbstract $skinObject ) {
		$this->skin = $skinObject;
	}
}

/**
 * Class OneaElatedClassSkinManager
 *
...
class OneaElatedClassSkinManager {
	...
	/**
	 * @see OneaElatedClassSkinManager::setSkin()
	 */
	...
	private function setSkin( $skinName = ELATED_PROFILE_SLUG ) {
		if ( $skinName !== '' ) {
			if ( file_exists( get_template_directory() . '/framework/admin/skins/' . $skinName . '/skin.php' ) ) {
				...
				$skinName = ucfirst( $skinName ) . esc_html__( 'Skin', 'onea' );
...
/**
 * Class OneaElatedClassSkinAbstract
 *
 * Abstract class that each skin class must extend
 */
abstract class OneaElatedClassSkinAbstract {
	/**
	 * @var string
	 */
	protected $skinName;
	...
	public function loadTemplatePart( $template, $params = array() ) {
		...
		if ( $template !== '' ) {
			include( onea_elated_find_template_path( 'framework/admin/skins/' . $this->skinName . '/templates/' . $template . '.php' ) );
		}
	}
	...
	public function setShortcodeJSParams() { ?>
		<script>
			window.eltdSCIcon = '<?php echo onea_elated_get_skin_uri() . '/assets/img/admin-logo-icon.png'; ?>';
			window.eltdSCLabel = '<?php echo esc_html( ucfirst( $this->skinName ) ); ?> <?php esc_html_e( 'Shortcodes', 'onea' ) ?>';
		</script>
	<?php }
...
class OneaElatedClassOptions {
	...
	public function getOptionValue( $key ) {
		...
		if ( array_key_exists( $key, $onea_elated_global_options ) ) {
			return $onea_elated_global_options[ $key ];
...
/*
   Class: OneaElatedClassAdminPage
   A class that initializes Elated Admin Page
*/
class OneaElatedClassAdminPage implements iOneaElatedInterfaceLayoutNode {
	public $layout;
	private $factory;
	public $slug;
	public $title;
	...
	function __construct( $slug = "", $title = "", $icon = "" ) {
		$this->layout  = array();
		$this->factory = new OneaElatedClassFieldFactory();
		$this->slug    = $slug;
		$this->title   = $title;
		$this->icon    = $icon;
	}
	...
	public function renderChild( iOneaElatedInterfaceRender $child ) {
		$child->render( $this->factory );
	}
}

/*
   Class: OneaElatedClassMetaBoxes
...
/*
   Class: OneaElatedClassMetaBox
   A class that initializes Elated Meta Box
*/
class OneaElatedClassMetaBox implements iOneaElatedInterfaceLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	public $title;
	...
	public function setTitle( $label ) {
		...
		return $onea_elated_global_Framework->getSkin()->getSkinLabel() . ' ' . $label;
	}
}

/*
   Class: OneaElatedClassTaxonomyOptions
   A class that initializes OneaElatedClass Taxonomy Options
*/
class OneaElatedClassTaxonomyOptions {
	private static $instance;
...
/*
   Class: OneaElatedClassTaxonomyOption
   A class that initializes OneaElatedClass Taxonomy Option
*/
class OneaElatedClassTaxonomyOption implements iOneaElatedInterfaceLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	...
	function __construct( $scope = "" ) {
		$this->layout  = array();
		$this->factory = new OneaElatedClassTaxonomyFieldFactory();
		$this->scope   = $scope;
	}
...
/*
   Class: OneaElatedClassUserOptions
   A class that initializes OneaElatedClass User Options
*/
class OneaElatedClassUserOptions {
	private static $instance;
	public $userOptions;
...
/*
   Class: OneaElatedClassUserOption
   A class that initializes OneaElatedClass User Option
*/
class OneaElatedClassUserOption implements iOneaElatedInterfaceLayoutNode {
	public $layout;
	private $factory;
	public $scope;
	...
	function __construct( $scope = "" ) {
		$this->layout  = array();
		$this->factory = new OneaElatedClassUserFieldFactory();
		$this->scope   = $scope;
	}
...
/*
   Class: OneaElatedClassDashboardOptions
   A class that initializes OneaElatedClass Dashboard Options
*/
class OneaElatedClassDashboardOptions {
	private static $instance;
...
/*
   Class: OneaElatedClassDashboardOption
   A class that initializes OneaElatedClass Dashboard Option
*/
class OneaElatedClassDashboardOption implements iOneaElatedInterfaceLayoutNode {
	public $layout;
	private $factory;
	
	function __construct() {
		$this->layout        = array();
		$this->factory       = new OneaElatedClassDashboardFieldFactory();
	}
...
if ( ! function_exists( 'onea_elated_init_framework_variable' ) ) {
	function onea_elated_init_framework_variable() {
		global $onea_elated_global_Framework;
		
		$onea_elated_global_Framework = OneaElatedClassFramework::get_instance();
		$eltdSkinManager                 = new OneaElatedClassSkinManager();
		$onea_elated_global_Framework->setSkin( $eltdSkinManager->getSkin() );
	}
	
	add_action( 'onea_elated_action_before_options_map', 'onea_elated_init_framework_variable' );
}
?>